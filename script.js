...
// - ANSWER_WORDS: Common words used as solutions (~2,315)
// - WORD_LIST: All valid words for validation (~14,855)
...
// Initialize Game
function init() {
    state.targetWord = ANSWER_WORDS[Math.floor(Math.random() * ANSWER_WORDS.length)].toUpperCase();
...
function resetGame() {
    // Reset state
    state = {
        targetWord: ANSWER_WORDS[Math.floor(Math.random() * ANSWER_WORDS.length)].toUpperCase(),
        currentRow: 0,
        currentTile: 0,
        gameOver: false,
        guesses: [],
